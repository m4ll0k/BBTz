...
def main()->None:
    ...
    if '.js' in url:
        ...
        if r.status_code == 200:
            js = beauty(r.content)
            rr = re.findall(regexs,js,re.I)
            if rr:
                for i in rr:
                    k = js.index(i)
                    a = js[js.index(i) - 100 :js.index(i) + 100]
                    a = a.replace(i,f'{R}{i}{E}')
                    n = a.split('\n')
                    print('line:%s%s%s%s -> %s%s\n%s'%(G,W,str(k),E,i,E,'-'*100))
